...
local helper_files : dir "${path_to_helpers}" files "Helper_*.do" // only the actual helper progs
...
if _rc == 0 { // if isic version info exists, otherwise cannot know which ISIC version to compare to
	...
	* Preserve harmonization file to read in ISIC universe, save
	...
	import delimited "${path_to_helpers}/isco_codes.txt", delimiter(comma) varnames(1) clear 
...
if `last_slash' == 0 local last_slash = strrpos("`current_filename'", "\")
