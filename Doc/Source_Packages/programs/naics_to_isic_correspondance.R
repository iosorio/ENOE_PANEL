...
get_base_dir <- function() {
  env <- Sys.getenv("ENOE_DOCS")
  if (nzchar(env)) return(env)
  this_file <- NULL
  if (!is.null(sys.frames()[[1]]$ofile)) this_file <- sys.frames()[[1]]$ofile
  if (is.null(this_file)) {
    cmd_args <- commandArgs(trailingOnly = FALSE)
    file_arg <- "--file="
    file <- cmd_args[grep(file_arg, cmd_args)]
    if (length(file) > 0) this_file <- sub(file_arg, "", file)
  }
  if (!is.null(this_file)) return(dirname(normalizePath(this_file)))
  return(getwd())
}

# ENOE_DOCS should point to the folder containing the input Excel files.
