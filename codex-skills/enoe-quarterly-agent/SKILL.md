...
4. For diagnosis, run `scripts/diagnose_stata_failure.py` on a provided artifact or let it inspect the latest run artifact.
5. For explanation tasks, read the repo docs first, then inspect the authoritative do-file or runner that implements the behavior.
...
- Surface repo-specific constraints in answers:
  - coverage currently runs through `2025Q3`,
