...
local path_in "/Users/Israel/OneDrive/Microdata/GLD/MEX/MEX_2014_ENOE-Q2/MEX_2014_ENOE_V01_M/Data/Stata"
local path_output "/Users/Israel/OneDrive/Microdata/GLD/MEX/MEX_2014_ENOE-Q2/MEX_2014_ENOE_V01_M_V06_A_GLD/Data/Harmonized"

*----------1.3: Database assembly-------------------s (if several) to have all elements needed to produce
* harmonized output in a single file
		use "`path_in'/COE1T214.dta", clear
	...
	use "`path_in'/COE2T214.dta", clear
	tostring (ent v_sel n_ren), gen(ent_str v_sel_str n_ren_str) format("%02.0f")
	tostring con, gen(con_str) format("%05.0f")
	tostring (n_hog h_mud), gen(n_hog_str h_mud_str) format("%01.0f")
	...
	* Overwrtwrite temp file
	...
	use "`path_in'/SDEMT214.dta", clear
	tostring (ent v_sel n_ren), gen(ent_str v_sel_str n_ren_str) format("%02.0f")
	tostring con, gen(con_str) format("%05.0f")
	tostring (n_hog h_mud), gen(n_hog_str h_mud_str) format("%01.0f")
	...
	* Overwrtwrite temp file
	...
	use "`path_in'/VIVT214.dta",clear
	drop p1-p3

	tostring (ent v_sel), gen(ent_str v_sel_str) format("%02.0f")
	tostring con, gen(con_str) format("%05.0f")

	egen casa_short = concat(ent con v_sel)

	* Merge with Individual files, keep only what matches
	merge 1:m casa_short using `mex_ind', keep(match) nogen force
	...
	* Merge with Individual files, keep only what matches
	merge 1:m casa_long using `mex_ind', keep(match) nogen force


	*the harmonization is for Q2 of the year , any other month will be put to missing.
	tab d_mes
	*replace d_mes=. if d_mes == 7 | d_mes == 3
	tab d_mes, missing
